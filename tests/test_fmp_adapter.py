...
class TestFMPAdapter:
    ...
    async def test_get_historical_prices_success(self, adapter: FMPAdapter, historical_payload: list[dict]) -> None:
        with patch("mi_sdk.providers.fmp.fmp_adapter.httpx.AsyncClient") as mock_client_class:
            ...
            result = await adapter.get_historical_prices(["AAPL"], "2026-07-16", 5)
    ...
    async def test_get_historical_prices_invalid_date(self, adapter: FMPAdapter) -> None:
        with pytest.raises(DataValidationError, match="Invalid as_of_date"):
            await adapter.get_historical_prices(["AAPL"], "07-16-2026", 5)

    async def test_get_historical_prices_invalid_lookback(self, adapter: FMPAdapter) -> None:
        with pytest.raises(DataValidationError, match="lookback_periods must be a whole number"):
            await adapter.get_historical_prices(["AAPL"], "2026-07-16", 0)

    async def test_get_historical_prices_http_error_raises_provider_unavailable(self, adapter: FMPAdapter) -> None:
        with patch("mi_sdk.providers.fmp.fmp_adapter.httpx.AsyncClient") as mock_client_class:
            mock_response = Mock()
            ...
            with pytest.raises(ProviderUnavailableError, match="Failed to fetch historical pricing"):
                await adapter.get_historical_prices(["AAPL"], "2026-07-16", 5)

    async def test_get_historical_prices_missing_data_returns_failed_response(self, adapter: FMPAdapter) -> None:
        with patch("mi_sdk.providers.fmp.fmp_adapter.httpx.AsyncClient") as mock_client_class:
    ...
    async def test_get_historical_prices_invalid_symbol_returns_symbol_not_found(self, adapter: FMPAdapter) -> None:
        with patch("mi_sdk.providers.fmp.fmp_adapter.httpx.AsyncClient") as mock_client_class:
            mock_historical_response = Mock()
            mock_historical_response.json.return_value = []
            mock_historical_response.raise_for_status = Mock()
            ...
            result = await adapter.get_historical_prices(["INVALID"], "2026-07-16", 5)
